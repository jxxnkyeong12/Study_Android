...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        btn_change.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                dao.showToast(MainActivity.this, "문자열보여주기");
    ...
    // static 키워드를 이용해서 멤버를 static 멤버로 만들기
    //static 이 붙으면 static멤버다 != static이 안붙으면 static멤버가 아니다.

    // 두가지 멤버가 있음 : Static 멤버, 인스턴스 멤버

    //static은 항상 메모리에 먼저 가있어야 함
    //인스턴스 멤버는 new를 통해서 인스턴스화 과정을 실행해야 메모리에 올라감.

   // static 메소드를 안드로이드에서 사용했을때 불편한점.
    //대부분의 안드로이드에서 사용되는 멤버들은 (Context, FragmentManager등) 인스턴스 멤버임.
    //static에서 구현을 하려면 하나씩 static으로 만들거나 해야하는 불편한점이 많음.(메모리용량많이씀)
    //MainActivity activity = new MainAcvtity()
    //아래 방식은 사용을 자제하자! (안된다고 무조건 static을 쓰지 말자)
 ...
 /*       Log.d("메인메소드", "mainMethod: ");
        ...
        btn_change.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
              ...
              //static 붙여주면 돼!  dao.showToast(MainActivity.this, "문자열보여주기");
                // Toast.makeText(MainActivity.this, "", Toast.LENGTH_SHORT).show();
